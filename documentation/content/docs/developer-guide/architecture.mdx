...
**Fluxo de Dados**:
```typescript
// Exemplo de conversão Revit→IFC
const ConversionFlow = () => {
  ...
  const upload = async () => {
    ...
    const response = await fetch('/api/models/generate-ifc', {
      method: 'POST',
      body: formData
    })
  }
  ...
  const ws = useWebSocket('/ws/conversion-progress')
  ws.on('progress', (data) => {
...
**Rotas Principais**:
- `/` - Landing page
- `/ifc-conversion` - Revit → IFC
- `/archicad-conversion` - Archicad → IFC
- `/model-transformation` - IFC → Revit
- `/conversion` - Interface genérica
- `/websocket-demo` - Demonstração WS
