...
export function ModelTransformation() {
  ...
  const startSecondConversion = async (
    ifcFilePath: string,
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:3000/models/convert-from-ifc',
        {
          method: 'POST',
          headers: {
      ...
      setError(
        `Erro na segunda conversão: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  };
  ...
  }, [jobs, currentJobIds, ifcPath, conversionTargets, startSecondConversion]);
    ...
    try {
      ...
      if (extension === '.rvt' || extension === '.pln') {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || 'Erro ao iniciar conversão para IFC',
          );
        }
        ...
        throw new Error(
          'Upload direto de IFC ainda não implementado. Primeiro converta de RVT/PLN para IFC.',
        );
      }
  ...
  const getConversionDescription = (): string => {
    ...
    const extension = getFileExtension(selectedFile.name);
    const _sourceFormat = extension.substring(1).toUpperCase();

    if (extension === '.ifc') {
      if (conversionTargets.length === 0) {
        return 'Selecione o formato de destino';
      }
      const targets = conversionTargets
        .map((t) => (t === 'revit' ? 'Revit' : 'Archicad'))
        .join(' e ');
      return `IFC → ${targets}`;
    } else if (extension === '.rvt') {
    ...
    <div className='mx-auto max-w-4xl space-y-6 p-6'>
      <div className='space-y-2'>
        ...
        <p className='text-gray-600'>
          Converta arquivos entre os formatos Revit (.rvt), Archicad (.pln) e
          IFC (.ifc).
        </p>
      </div>
      ...
      {currentJobIds.length === 0 ? (
        <div className='space-y-6'>
          ...
          <div className='flex gap-4'>
            ...
            <button
              ...
              className='rounded-md border border-gray-300 bg-white px-4 py-3 text-base font-semibold text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 cursor-pointer disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-500'
            >
              Limpar
            </button>
          </div>
        ...
        <div className='space-y-6'>
          {currentJobIds.map((jobId, _index) => {
            ...
            const shouldShowDownload =
              allJobsCompleted || currentJobIds.length === 1;
