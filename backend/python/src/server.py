...
def _cleanup_worker() -> None:
    logger.info(
        "Job cleanup worker started (interval=%ss, ttl=%s minutes)",
...
def _handle_plugin_job_event(plugin: PluginType, payload: Dict[str, Any]) -> None:
    ...
    if event == "conversion_started":
        ...
        if "fileSize" in result_info and isinstance(
            result_info["fileSize"], (int, float)
        ):
...
def health():
    ...
    plugin_states = {
        plugin.value: "connected"
        if plugin_ws_server.is_plugin_connected(plugin)
        else "disconnected"
...
def convert_archicad_to_ifc():
    ...
    try:
        ...
        raw_file_path = (
            request.form.get("filePath") or request.form.get("plnPath") or ""
        )
        file_path_field = raw_file_path.strip() or None

        if not incoming_file and not file_path_field:
            return jsonify(
                {"error": "No file payload provided. Supply a file or filePath."}
            ), 400
        ...
        if file_path_field:
            ...
            if not resolved_candidate.exists():
                return jsonify(
                    {"error": f"Provided filePath does not exist: {resolved_candidate}"}
                ), 400

            if not resolved_candidate.is_file():
                return jsonify(
                    {"error": f"Provided filePath is not a file: {resolved_candidate}"}
                ), 400

            if resolved_candidate.suffix.lower() != ".pln":
                return jsonify(
                    {"error": "Invalid file format. Only .pln files are accepted"}
                ), 400

            input_path = str(resolved_candidate)
            original_filename = (
                request.form.get("originalFilename")
                or resolved_candidate.name
                or f"{job_id}.pln"
            )
        ...
        if requested_output_path:
            ...
            try:
                ...
                logger.warning(
                    f"Could not ensure output directory for job {job_id}: {exc}"
                )
        else:
        ...
        initial_message = (
            "Awaiting upload"
            if staging_strategy == "direct-upload"
            else "Using staged PLN path from Node backend"
        )
...
def trigger_revit_conversion():
    ...
    try:
        ...
        download_name = (
            data.get("downloadName") or f"{Path(ifc_path).stem or job_id}.rvt"
        )
        ...
        if requested_output_path:
            ...
            try:
                ...
                logger.warning(
                    "Could not ensure output directory for job %s: %s", job_id, exc
                )
        else:
            output_path = str(_output_path(job_id, download_name))
        ...
        ws_client.send_progress(
            job_id, 0, "queued", "Conversion request accepted by Python bridge"
        )
        _persist_job(
            job_id,
            {
...
def convert_ifc_to_archicad():
    ...
    try:
        ...
        if resolved_candidate.suffix.lower() != ".ifc":
            return jsonify(
                {"error": "Invalid file format. Only .ifc files are accepted"}
            ), 400
        ...
        if not dispatched:
            error_msg = (
                "Failed to dispatch IFC to PLN conversion command to Archicad plugin"
            )
            logger.error(error_msg)
            _persist_job(
                job_id,
...
def download_job_output(job_id: str):
    ...
    if not download_path:
        return jsonify(
            {"error": "No downloadable artifact registered for this job"}
        ), 404
...
# Set callback to resend plugin statuses when Node.js connects/reconnects
def on_nodejs_connected():
    """Called when WebSocket connection to Node.js is established"""
    logger.info("Node.js WebSocket connected - resending plugin statuses")
    plugin_ws_server.resend_all_plugin_statuses()


ws_client.set_on_connect_handler(on_nodejs_connected)
